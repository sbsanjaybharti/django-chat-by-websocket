...
from apps.Message.models import Message, Sender, Reciever
from apps.Message.exceptions import ClientError
from apps.Message.utils import catch_client_error, get_room_or_error
from onlinechatting.settings import NOTIFY_USERS_ON_ENTER_OR_LEAVE_ROOMS, MSG_TYPE_ENTER, MSG_TYPE_LEAVE
...
def ws_disconnect(message):
    ...
    for room_id in message.channel_session.get("rooms", set()):
        try:
            room = Message.objects.get(pk=room_id)
...
def chat_join(message):
    ...
    profile_image = "https://www.gravatar.com/avatar/" + message.user.email + "?d=identicon"
...
def chat_send(message):
    ...
    sender_obj.save()
    reciever_obj = Reciever.objects.create(
        message=message_obj,
        user=message.user,
        group_id=message["room"],
        read = False
